...
- any broad consequence from S2165: frontier-local width scheduling only for
  the restricted nonempty-fanin normalized-view route.  The schedule is the
  maximum recurrence width of the selected finite frontier, clamped by one;
  it changes no threshold, switching constant, stage budget, geometric
...
- any broad consequence from S2166: actual normalized-frontier width
  ...
  P-vs-NP;
- any broad consequence from S2167: representative frontier layers only for the
  restricted nonempty-fanin normalized-view route.  A representative layer is a
  free-multiplicity same-membership list for one recursive frontier; entry
  products may use the representative count while the final tree still certifies
  the original parent-merged frontier formula.  This is not semantic formula
  minimization, threshold redesign, arbitrary-class width synthesis, full B4,
...
- any broad consequence from S2168: exactly a finite restricted nonempty-fanin
  ...
  implication;
- any broad consequence from S2169: synthesized duplicate-free representative
  frontier layers only for the restricted nonempty-fanin normalized-view
  route.  `dedupRepresentativeFrontier` removes syntactic duplicate gate
  copies from one raw recursive frontier; its consumers synthesize the
  representative layer lists, membership proofs, and count obligations, but
  still take the class depth/size data and every per-level ambient entry
  product as hypotheses.  Decidable syntactic equality for raw formulas is
  structural bookkeeping.  This is not semantic formula minimization,
...
- any broad consequence from S2171: the new schedule changes only the geometric
  stage divisor from `64*m` to `32*m`, uniformly matching the existing ratio
  coefficient.  The route remains conditional on the corresponding
  representative-layer/class-bound/product inputs; supplied-layer variants
  take layers explicitly; dedup variants synthesize `dedupRepresentativeFrontier`;
  normalized-width variants derive width bounds internally from class-depth
  data.  Stage budget `2`, final-tree payload, and `t(d,s)=S(d)*(s-1)` are unchanged.
...
- any broad consequence from S2175 Gate B Route C1: this is bounded schedule
  arithmetic only (historically over factor-8 counting), deriving a sufficient
  coefficient-`17` form plus a parallel divisor-`17` schedule and SimpleDNF
  term-canonical finite counting/representative-layer packaging at exact ambient
  `9826`.  Stage budget remains `s=2`, and `t(d,s)=S(d)*(s-1)` is unchanged.
  This is NOT an unqualified switching-lemma improvement, full B4, PHP or
...
- any broad consequence from S2176: this threads factor-4 SimpleDNF
  term-canonical counting through the real auto-collapse consumer path
  (`GeneratedRefinedCollapse` / `GeneratedOneStepDepthReduction` /
  `BeatArith` / product packaging) and pins coefficient-`16` schedule arithmetic
  with ambient `8192 = 2*(16)^2*16` packaging.  The exact affine condition is
...
|---|---|---|
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.frontierMaxRecurrenceWidth` / `frontierRecurrenceWidthSchedule` / `frontierRecurrenceWidthSchedule_pos` | `propext`, `Quot.sound` | S2165 frontier-local schedule definitions and positivity |
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.normalizedFrontierMinimalLayer_width_le_frontierRecurrenceWidthSchedule` | `propext`, `Classical.choice`, `Quot.sound` | S2165 restricted normalized-frontier width validity |
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.normalizedFrontier_geometricCollapse_finalTree_tightEntry_frontierWidth` / `allNormalizedFrontiers_geometricCollapse_finalTree_tightEntry_frontierWidth` | `propext`, `Classical.choice`, `Quot.sound` | S2165 frontier-local consumer wrappers |
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.sharedWitness25` | none | S2165 finite ambient-2^25 witness |
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.sharedWitness25_nonemptyFanin` | `propext` | S2165 restricted-class witness pin |
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.sharedWitness25_formulaSize` / `sharedWitness25_depth` / `sharedWitness25_recurrenceWidth` | `propext`, `Quot.sound` | S2165 structural witness pins |
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.sharedWitness25_frontierGateCount_zero` / `sharedWitness25_frontierGateCount_one` / `sharedWitness25_frontierGateCount_two` | `propext`, `Quot.sound` | S2165 frontier-count pins |
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.sharedWitness25_frontierRecurrenceWidthSchedule_zero` / `sharedWitness25_frontierRecurrenceWidthSchedule_one` / `sharedWitness25_frontierRecurrenceWidthSchedule_two` / `sharedWitness25_recurrenceWidthSchedule` | `propext`, `Quot.sound` | S2165 frontier/local and root schedule pins |
| `PvNP.FormulaRecursiveSyntacticTerminalNormalizedViewRoute.sharedWitness25_frontierSchedule_strict_level1` / `sharedWitness25_frontierSchedule_strict_level2` | `propext`, `Quot.sound` | S2165 strict schedule comparison pins |
...
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.RepresentativeFrontierLayer` | none | S2167 representative-layer predicate |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.eval_parentMerge_congr_of_mem_iff` / `eval_restrict_parentMerge_congr_of_mem_iff` | `propext`, `Quot.sound` | S2167 parent-merge evaluation congruence |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.representativeMinimalLayer` / `representativeMinimalLayer_originalFormula` / `representativeMinimalLayer_gateCount` | `propext`, `Classical.choice`, `Quot.sound` | S2167 representative minimal-layer packaging |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.representativeMinimalLayer_width_le_normalizedFrontierWidthSchedule` | `propext`, `Classical.choice`, `Quot.sound` | S2167 representative width validity under S2166 schedule |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.representative_length_pos` | `propext`, `Quot.sound` | S2167 representative nonempty pin |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.RepresentativeNormalizedViewClassDepthFinalTreeAt` | `propext`, `Classical.choice`, `Quot.sound` | S2167 representative final-tree payload |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.representativeFrontier_geometricCollapseWithSuppliedWidth_finalTree_tightEntry` / `allRepresentativeFrontiers_geometricCollapseWithSuppliedWidth_finalTree_tightEntry` | `propext`, `Classical.choice`, `Quot.sound` | S2167 supplied-width representative consumers |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.representativeFrontier_geometricCollapse_finalTree_tightEntry_normalizedWidth` / `allRepresentativeFrontiers_geometricCollapse_finalTree_tightEntry_normalizedWidth` | `propext`, `Classical.choice`, `Quot.sound` | S2167 class-derived representative consumers |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19` / `dupSquareRepLayer19` | none | S2167 finite ambient-2^19 witness and layers |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19_nonemptyFanin` | `propext` | S2167 restricted-class witness pin |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19_syntacticDNF_not_simple` | `propext`, `Quot.sound` | S2167 non-simple raw-expansion pin |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19_formulaSize` / `dupSquareWitness19_depth` / `dupSquareWitness19_recurrenceWidth` | `propext`, `Quot.sound` | S2167 structural witness pins |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19_frontierGateCount_zero` / `dupSquareWitness19_frontierGateCount_one` / `dupSquareWitness19_frontierGateCount_two` | `propext`, `Quot.sound` | S2167 raw frontier-count pins |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19_normalizedWidth` | `propext`, `Classical.choice`, `Quot.sound` | S2167 normalized-width non-degeneracy pin |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19_normalizedFrontierWidthSchedule_zero` / `dupSquareWitness19_normalizedFrontierWidthSchedule_one` / `dupSquareWitness19_normalizedFrontierWidthSchedule_two` | `propext`, `Classical.choice`, `Quot.sound` | S2167 normalized-frontier schedule value pins |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19_representativeLayers` | `propext`, `Quot.sound` | S2167 representative-layer correctness pin |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareRepLayer19_length` | none | S2167 representative count pin |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupSquareWitness19_entryProduct_eq` | `propext` | S2167 exact entry-product pin |
...
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.formulaBEq` / `formulaListBEq` / `formulaBEq_eq_true_iff` / `instDecidableEqBDFormula` | `propext`, `Quot.sound` | S2169 decidable syntactic equality for raw formulas |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dedupRepresentativeFrontier` | `propext`, `Quot.sound` | S2169 synthesized duplicate-free representative layer |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dedupRepresentativeFrontier_representative` | `propext`, `Classical.choice`, `Quot.sound` | S2169 automatic representative-layer membership |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dedupRepresentativeFrontier_nodup` | `propext`, `Quot.sound` | S2169 duplicate-freeness pin |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dedupRepresentativeFrontier_length_le_frontierGateCount` / `dedupRepresentativeFrontier_length_le_formulaSize` | `propext`, `Quot.sound` | S2169 synthesized count bounds |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dedupRepresentativeFrontier_length_pos` | `propext`, `Classical.choice`, `Quot.sound` | S2169 synthesized nonempty pin |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.allDedupFrontiers_geometricCollapseWithSuppliedWidth_finalTree_tightEntry` | `propext`, `Classical.choice`, `Quot.sound` | S2169 supplied-width synthesized consumer |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dedupFrontier_geometricCollapse_finalTree_tightEntry_normalizedWidth` / `allDedupFrontiers_geometricCollapse_finalTree_tightEntry_normalizedWidth` | `propext`, `Classical.choice`, `Quot.sound` | S2169 class-derived synthesized consumers |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupCubeWitness19_dedupFrontier_zero` / `dupCubeWitness19_dedupFrontier_one` / `dupCubeWitness19_dedupFrontier_two` / `dupCubeWitness19_dedupFrontier_three` | `propext`, `Classical.choice`, `Quot.sound` | S2169 synthesized-equals-supplied layer pins |
...
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.representativeFrontier_geometricCollapseWithSuppliedWidth_finalTree_tightEntry32` / `representativeFrontier_geometricCollapse_finalTree_tightEntry_normalizedWidth32` / `allDedupFrontiers_geometricCollapse_finalTree_tightEntry_normalizedWidth32` / `dedupFrontier_geometricCollapse_finalTree_tightEntry_normalizedWidth32` | `propext`, `Classical.choice`, `Quot.sound` | Public coefficient-32 aliases for the existing S2170 consumers |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.dupCubeWitness19_tightEntry_product_eq` / `dupCubeWitness19_oldEntry_fails_at_2pow18` | `propext` | Public exact coefficient-32 product and former coefficient-64 failure pins at ambient `2^18` |
| `PvNP.FrozenProductScheduleRatio.geometricSchedule32` / `geometricSchedule32_length` / `geometricSchedule32_budgets` / `geometricSchedule32_treeBudget` | none | S2171 uniform coefficient-32 schedule bookkeeping |
| `PvNP.FrozenProductScheduleRatio.geometricSchedule32_regime` / `geometric_regime_of_bound32` | `propext` | S2171 ratio-regime and ambient-product bounds |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.RepresentativeNormalizedViewClassDepthFinalTreeAtUniform32` | `propext`, `Classical.choice`, `Quot.sound` | S2171 uniform-32 representative payload |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.representativeFrontier_geometricCollapseWithSuppliedWidth_finalTree_uniform32` / `representativeFrontier_geometricCollapse_finalTree_uniform32_normalizedWidth` / `allDedupFrontiers_geometricCollapse_finalTree_uniform32_normalizedWidth` / `dedupFrontier_geometricCollapse_finalTree_uniform32_normalizedWidth` | `propext`, `Classical.choice`, `Quot.sound` | S2171 four public consumers |
...
| `PvNP.ScheduledAutoCollapse.BeatArith4` / `ValidFrom4` | `propext` | Explicit aliases for the in-place real factor-4 beat and validity interfaces |
| `PvNP.FrozenProductScheduleRatio.ratio_beat4` | `propext`, `Classical.choice`, `Quot.sound` | S2176 coefficient-16 sufficient form of factor-4 affine beat |
| `PvNP.FrozenProductScheduleRatio.RatioRegime16` / `RatioRegime16.mono` / `RegimeFrom16` | none | S2176 parallel coefficient-16 regime predicates |
| `PvNP.FrozenProductScheduleRatio.ratioRegime16_beat` / `regimeFrom16_validFrom` / `regimeFrom16_validFrom4` | `propext`, `Classical.choice`, `Quot.sound` | S2176 coefficient-16 regime consumers |
| `PvNP.FrozenProductScheduleRatio.geometricSchedule16` / `geometricSchedule16_length` / `geometricSchedule16_budgets` / `geometricSchedule16_treeBudget` | none | S2176 divisor-16 schedule bookkeeping |
| `PvNP.FrozenProductScheduleRatio.geometricSchedule16_regime` / `geometric_regime_of_bound16` | `propext` | S2176 divisor-16 regime and ambient-`8192` product bounds |
| `PvNP.FormulaRecursiveSyntacticTerminalRepresentativeFrontierRoute.RepresentativeNormalizedViewClassDepthFinalTreeAtUniform16` / `representativeFrontier_geometricCollapseWithSuppliedWidth_finalTree_uniform16` / `representativeFrontier_geometricCollapse_finalTree_uniform16_normalizedWidth` / `dedupFrontier_geometricCollapse_finalTree_uniform16_normalizedWidth` / `allDedupFrontiers_geometricCollapse_finalTree_uniform16_normalizedWidth` | `propext`, `Classical.choice`, `Quot.sound` | S2176 uniform-16 representative consumers |
